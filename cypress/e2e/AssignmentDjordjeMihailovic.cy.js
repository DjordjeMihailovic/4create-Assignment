describe('Assignment Automation Test', () => {
  ...
  it('Automated Steps', () => {
    ...
    for (let i = 0; i < 4; i++) {
      cy.get('.product:contains("Brocolli")').contains('ADD TO CART', { timeout: 10000 }).click()      
    }
    ...
    otherItems.forEach(function (value) {                                                // of those items was of testers choice; Otherwise, pseudocode for selecting truly random items would have been implemented:
      cy.get(`.product:contains(${value})`).contains('ADD TO CART').click()              // cy.get('.product').then($products => { cy.wrap($products).eq(Math.floor(Math.random() * $products.length)) ...
    })                                                                                
    ...
    cy.get('.cart-item')
      ...
      .should('contain', '4 No')
      .next()
      .should('contain', 'Carrot')
      .should('contain', '1 No')                        // will be cleaned
      .next()
      .should('contain', 'Tomato')
      .should('contain', '1 No')
      .next()
    ...
    cy.get('.product-price').each(($el) => {
      ...
      const minIndex = priceList.indexOf(minPrice)                                                 // NOTE: The solution makes a list of all unique prices; it provides the min and max price and a 3rd random one
      const maxIndex = priceList.indexOf(maxPrice)                                                 // exluding the min and max; it then adds the first element to the cart respectively matching the price value

      priceList.splice(minIndex, 1)
      priceList.splice(maxIndex, 1)

      let randomOtherIndex = [Math.floor(Math.random() * priceList.length)]
      let randomOtherPrice = priceList[randomOtherIndex]

      cy.get(`.product:contains(${minPrice})`).first().contains('ADD TO CART').click()
      cy.get(`.product:contains(${maxPrice})`).first().contains('ADD TO CART').click()
      cy.get(`.product:contains(${randomOtherPrice})`).first().contains('ADD TO CART').click()
    ...
    cy.get('.promoInfo', { timeout: 10000 }).contains('Invalid code').should('exist')
    ...
    cy.url({ timeout: 10000 }).should('eq', HomeURL)
    ...
    cy.screenshot('ACTIONS_screenshoot', { capture: 'viewport' })
    ...
    cy.go('back')
    cy.screenshot('HomePage_screenshoot', { capture: 'viewport' })
    ...
    // 11. Drag and drop "Drag me" element to "Drop here" box
      ...
      cy.get('#draggable').drag('#droppable', ({force: true}))                                     // Package '@4tw/cypress-drag-drop' was imported for a reliable solution
    ...
    cy.on('window:alert', ($alert) => {
      ...
      expect(alertMessage).to.equal('Well done you clicked on the link!')                          // Cypress is automaticaly clicking OK in case where it is the only option for an Alert message
        ...
        cy.writeFile('cypress/fixtures/alertMessage.json', { alertMessage })
    })
